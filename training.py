...
plt.title("Phân phối tập dữ liệu huấn luyện")
plt.xlabel("Số lớp")
...
def preprocessing(img):
    img = grayscale(img)
    img = equalize(img)
    img = img / 255 # chuẩn hóa hình ảnh
    return img
...
### định hình lại dữ liệu vào kênh 1
...
model=keras.models.load_model('traffic_sign_model.h5')      # tải mô hình từ thư mục
#####Mã ma trận nhầm lẫn####
y_pred = model.predict(X_test)
y_pred = np.argmax(y_pred, axis=1)

cm=confusion_matrix(Y_tests,y_pred)     # ma trận nhầm lẫn
